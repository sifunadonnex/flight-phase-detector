...
class FlightPhaseDetector:
    ...
    def load_data(self):
        ...
        required_cols = ['AIRSPEED  L', 'AIRSPEED R', 'ALTITUDE L', 'ALTITUDE R']
    ...
    def _calculate_derived_parameters(self):
        ...
        # Average airspeed from left and right sensors
        self.df['AIRSPEED_AVG'] = (self.df['AIRSPEED  L'] + self.df['AIRSPEED R']) / 2
        
        # Average altitude from left and right sensors
        self.df['ALTITUDE_AVG'] = (self.df['ALTITUDE L'] + self.df['ALTITUDE R']) / 2
        
        # Average engine torque (handle potential column name variations)
        tq1_col = 'TQ 1' if 'TQ 1' in self.df.columns else 'TQ 1 '
        tq2_col = 'TQ 2' if 'TQ 2' in self.df.columns else 'TQ 2 '
        self.df['TQ_AVG'] = (self.df[tq1_col] + self.df[tq2_col]) / 2
    ...
    def _classify_phase(self, idx):
        ...
        has_flown = False
        if idx > 0:
            # Check if we've previously been at cruise altitude or high speed
